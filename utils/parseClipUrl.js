/**
 * @typedef {'youtube' | 'rutube' | 'vimeo' | 'vk'} ClipProvider
 * @typedef {{ provider: ClipProvider, videoId: string, embedUrl: string, thumbnailUrl?: string }} ParsedClip
 */
...
/** @returns {ParsedClip|null} */
export function parseClipUrl(input) {
...
export function getClipProviderLabel(provider) {
  const labels = {
    youtube: 'YouTube',
    rutube: 'RuTube',
    vimeo: 'Vimeo',
...
export function getTrackClipUrl(track) {
  if (!track || typeof track !== 'object') return '';
  return String(track.clipUrl || track.youtubeUrl || '').trim();
}

export { buildYoutubeEmbedUrl, parseYoutube, parseRutube, parseVimeo, parseVk };
